...
- Run `./setup_proxmox.sh 1.2.3.4 /path/to/your/public/.ssh/key`. This script will:
...
- After running the script, it will:
  ...
  - Show you a port that was randomly chosen port for `ssh`. Suppose it is `1234`. Save it in `~/.ssh/config`:
    ...
    Host proxmox
      ...
      Port 1234
    ```
- Run `./verify_proxmox.sh 1.2.3.4 /path/to/your/private/.ssh/key` to verify that the SSH access is set-up correctly.
