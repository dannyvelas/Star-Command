...
func upsertTerraformConstraint(terraformFilePath, desiredConstraint string) error {
	src, err := os.ReadFile(terraformFilePath)
	if err != nil {
		return fmt.Errorf("error reading terraform file at %s: %v", terraformFilePath, err)
	}

	newFileByes, err := transformTerraformVersion(src, terraformFilePath, desiredConstraint)
	...
	if err := os.WriteFile(terraformFilePath, newFileByes, 0o644); err != nil {
...
func locateTerraform(ctx context.Context, desiredConstraint string) (string, func(), error) {
...
func applyTerraform(ctx context.Context, config any, terraformFilePath, execPath string) error {
	tf, err := tfexec.NewTerraform(filepath.Dir(terraformFilePath), execPath)
	if err != nil {
